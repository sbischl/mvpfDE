taxReform2000 <- function (bootstrap_replication = 0, year_difference = 3) {
  ...
  elasticity_of_taxable_income <- c(estimatates$eti_1year, estimatates$eti_2year, estimatates$eti_3year)[year_difference]
  ...
  fiscal_externality_before <-
    marginal_top_tax_rate_before / (1 - marginal_top_tax_rate_before) * estimatates$pareto_coefficient_2002 * elasticity_of_taxable_income

  fiscal_externality_after <-
    marginal_top_tax_rate_after / (1 - marginal_top_tax_rate_after) * estimatates$pareto_coefficient_2002 * elasticity_of_taxable_income
